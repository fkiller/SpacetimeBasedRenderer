...
interface UIButton {
  id: WatchAction;
  label: string;
  rect: { x: number; y: number; w: number; h: number }; // uv space 0..1
}
...
export class WatchUI {
  ...
  private data: WatchData = { count: 0, selectedLabel: '-', mass: 1 };
  private dirty = true;
  private hovered: WatchAction | null = null;
  ...
  setData(data: Partial<WatchData>) {
    this.data = { ...this.data, ...data };
    this.dirty = true;
  }

  setHover(action: WatchAction | null) {
  ...
  private drawButton(button: UIButton) {
    const { ctx } = this;
  ...
  private rectToPx(rect: { x: number; y: number; w: number; h: number }): [number, number, number, number] {
    return [
  ...
  private findButtonAtUV(uv: Vector2): WatchAction | null {
